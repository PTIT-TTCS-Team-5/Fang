...
| **Code entry point** | [`cv_parser.py:CVParserOrchestrator.parse()`](../app/services/cv_parser.py#L240) |
| **Prompt/template location** | [`cv_parser_adapters.py:CV_PARSE_PROMPT`](../app/services/cv_parser_adapters.py#L18-L49) — hằng số inline, tiếng Anh |
| **Anthropic variant** | [`cv_parser_adapters.py:ANTHROPIC_SCHEMA_PROMPT`](../app/services/cv_parser_adapters.py#L52-L56) — thêm toàn bộ JSON schema vào prompt (không có native structured output) |
...
| **Output contract** | Pydantic model [`ParsedCV`](../app/models/cv_models.py#L97) (`candidateInfo`, `education`, `experience`, `skills`, `certificates`, `languages`, `summary`, `rawText`, `expectedSalaryMin/Max`, `parserSelfReport`) |
...
| **Fallback logic** | Các lite tier (1–3) luôn được thử tuần tự. Các pro tier (4–5) chỉ được kích hoạt thông qua [`ProTierGate`](../app/services/cv_parser.py#L219-L237): escalate nếu có ít nhất một lite tier trả về kết quả chất lượng thấp (low-quality output); bỏ qua nếu có ≥2 lỗi infra. Hỗ trợ intra-provider model candidate fallback (ví dụ: `gemini-flash` → `gemini-3.1-flash` → `gemini-2.5-flash`). |
| **Retry** | Sử dụng [`tenacity`](../app/services/cv_parser.py#L358-L371): chỉ áp dụng cho `TransientProviderError`, 3 lần thử, exponential backoff từ 2–8 giây (có thể cấu hình) |
| **Validation/fallback** | 4-check quality gate ([`_build_quality_gate`](../app/services/cv_parser.py#L138-L187)): (1) rawText ≥ 120 ký tự, (2) có tín hiệu nhận diện ứng viên (candidate identity signal), (3) có ≥ 1 phần không trống, (4) parser self-confidence ≥ 0.55. Sử dụng cấu hình `extra="ignore"` của Pydantic. |
...
| **Tests/evals** | [`tests/unit/unit_test_parser_policy.py`](../tests/unit/unit_test_parser_policy.py) — kiểm tra logic của quality gate + orchestrator. Chưa thực hiện eval đối với kết quả đầu ra của LLM. |
...
| **Category** | Embedding |
| **Code entry point** | [`embedding.py:embed_chunks()`](../app/services/embedding.py#L30) |
...
| **Dimensions** | Mặc định là `1536` ([`config.py:embedding_dim`](../app/core/config.py#L8)), có thể ghi đè (overridable) trên từng lệnh gọi. Các kỹ năng trong NMAIex sử dụng `256` chiều (dims). Có hỗ trợ Matryoshka-compatible truncation thông qua tham số `output_dimensionality`. |
| **Batch size** | `32` (có thể cấu hình qua `embedding_batch_size`) |
| **Storage** | Kiểu dữ liệu pgvector `halfvec` tại cột `AIDOCUMENTCHUNK.embedding`. Được tuần tự hóa (serialized) thông qua [`persistence.py:_serialize_embedding()`](../app/services/persistence.py#L195). |
...
| **Tests/evals** | [`tests/unit/unit_test_embedding.py`](../tests/unit/unit_test_embedding.py) — 7 trường hợp kiểm thử sử dụng mock `FakeGeminiClient`. |
...
| **Code entry point** | [`chunking.py:process_document_to_chunks()`](../app/services/chunking.py#L50) |
...
| **Tests** | [`tests/unit/unit_test_chunking.py`](../tests/unit/unit_test_chunking.py) — 2 trường hợp kiểm thử (test cases). |
...
| **Code entry point** | [`rag_query.py:process_chat_query()`](../app/services/rag_query.py#L252) → [`routes_chat.py:POST /chat/query`](../app/api/routes_chat.py#L47) |
| **Prompt/template location** | [`rag_query.py:_build_system_prompt()`](../app/services/rag_query.py#L123-L193) — inline, tiếng Việt |
...
| **Output contract** | Pydantic [`ChatQueryResponse`](../app/models/chat.py#L50): `conversationId`, `messageId`, `response`, `model`, `modelMode`, `fallbackPath`, `latencyMs`, `topK`, `contextWarning` |
...
| **Code entry point** | [`routes_chat.py:POST /chat/conversations/{id}/summarize`](../app/api/routes_chat.py#L165) |
| **Prompt/template** | Viết inline tại [`routes_chat.py:L192–L202`](../app/api/routes_chat.py#L192-L202) — tiếng Việt: "Tóm tắt cuộc hội thoại HR-AI dưới đây thành bản rút gọn, giữ lại các điểm quan trọng về ứng viên, đánh giá, và kết luận." |
...
| **Model** | [`settings.context_summarization_model`](../app/core/config.py#L44) — mặc định là `"gemini-flash"` |
...
| **Category** | Generation (summarization) |
| **Code entry point** | [`routes_chat.py:POST /chat/conversations/{id}/branch-new`](../app/api/routes_chat.py#L234) |
| **Prompt/template** | Viết inline tại [`routes_chat.py:L260–L270`](../app/api/routes_chat.py#L260-L270) — tiếng Việt: "Tóm tắt cuộc hội thoại HR-AI dưới đây thành bản rút gọn để mang sang hội thoại mới." |
...
| **Code entry point** | [`nmaiex_mapper_service.py:map_string_to_province_id()`](../app/services/nmaiex_mapper_service.py#L23) |
| **Prompt/template** | Viết inline tại [`nmaiex_mapper_service.py:L38–L50`](../app/services/nmaiex_mapper_service.py#L38-L50) — tiếng Việt. Thực hiện inject danh mục tỉnh thành (province catalog) thực tế từ DB lúc runtime. |
...
| **Output contract** | Pydantic [`ProvinceMappingResult`](../app/models/nmaiex_schemas.py#L26): `prov_id: Optional[str]` |
...
| **Category** | Inference / Mapping |
| **Code entry point** | [`nmaiex_mapper_service.py:map_skills()`](../app/services/nmaiex_mapper_service.py#L71) |
| **Prompt/template** | Viết inline tại [`nmaiex_mapper_service.py:L92–L106`](../app/services/nmaiex_mapper_service.py#L92-L106) — tiếng Việt. Thực hiện chèn đầy đủ danh mục kỹ năng (skill catalog) từ DB. |
...
| **Output contract** | Pydantic [`SkillMappingResult`](../app/models/nmaiex_schemas.py#L15): `matched_ids: list[int]`, `unmatched_texts: list[str]` |
...
| **Category** | Embedding |
| **Code entry point** | [`nmaiex_mapper_service.py:embed_and_store_raw_skills()`](../app/services/nmaiex_mapper_service.py#L142) |
...
| **Models/modes** | `gemini-embedding-001` ở số chiều `256` (lấy từ [`nmaiex_config.py:nmaiex_skill_embedding_dims`](../app/core/nmaiex_config.py#L34)) |
...
| **Code entry point** | [`nmaiex_mapper_service.py:normalize_proficiency()`](../app/services/nmaiex_mapper_service.py#L228) |
| **Prompt/template** | Hằng số cấp module [`_PROFICIENCY_SYSTEM_PROMPT`](../app/services/nmaiex_mapper_service.py#L211-L225) — tiếng Việt. Ánh xạ sang các cấp độ `BASIC | INTERMEDIATE | ADVANCED | FLUENT | NATIVE` kèm theo các ví dụ. |
...
| **Code entry point** | [`nmaiex_ranking_service.py:compute_skill_score()`](../app/services/nmaiex_ranking_service.py#L226) |
...
| **Code entry point** | [`nmaiex_ranking_service.py:rank_candidates_for_job()`](../app/services/nmaiex_ranking_service.py#L368) |
...
| **Code entry point** | [`synthetic_data/run_pipeline.py`](../synthetic_data/run_pipeline.py) → [`generator.py`](../synthetic_data/generator.py) |
| **Prompt/template** | [`synthetic_data/prompts.py`](../synthetic_data/prompts.py): `CV_SYSTEM_PROMPT` (L27–47), `JOB_SYSTEM_PROMPT` (L94–123) — tiếng Việt |
...
| **Code entry point** | [`scripts/retry_nmaiex_candidate_enrichment.py`](../scripts/retry_nmaiex_candidate_enrichment.py) |
...
Theo quyết định trong tài liệu [`FANG_NEXT_PHASE_DECISIONS.md`](FANG_NEXT_PHASE_DECISIONS.md#L19-L22), phần trò chuyện `JobApplication` sẽ chuyển đổi từ cơ chế cố định chunk-RAG sang sử dụng ngữ cảnh full CV markdown.
...
|---|---|---|---|---|---|
| P1 | CV Parse Prompt | [`cv_parser_adapters.py`](../app/services/cv_parser_adapters.py#L18-L49) | 18–49 | tiếng Anh | Hằng số inline `CV_PARSE_PROMPT` |
| P2 | Anthropic Schema Prompt (extends P1) | [`cv_parser_adapters.py`](../app/services/cv_parser_adapters.py#L52-L56) | 52–56 | tiếng Anh | Hằng số inline `ANTHROPIC_SCHEMA_PROMPT` |
| P3 | HR Co-pilot System Prompt | [`rag_query.py`](../app/services/rag_query.py#L123-L193) | 123–193 | tiếng Việt | Bộ dựng động `_build_system_prompt()` |
| P4 | Chat Summarization Prompt | [`routes_chat.py`](../app/api/routes_chat.py#L192-L202) | 192–202 | tiếng Việt | Viết inline trong endpoint handler |
| P5 | Chat Branch Summarization Prompt | [`routes_chat.py`](../app/api/routes_chat.py#L260-L270) | 260–270 | tiếng Việt | Viết inline trong endpoint handler |
| P6 | Province Mapping Prompt | [`nmaiex_mapper_service.py`](../app/services/nmaiex_mapper_service.py#L38-L50) | 38–50 | tiếng Việt | Viết inline trong hàm (function) |
| P7 | Skill Mapping Prompt | [`nmaiex_mapper_service.py`](../app/services/nmaiex_mapper_service.py#L92-L106) | 92–106 | tiếng Việt | Viết inline trong hàm (function) |
| P8 | Proficiency Normalization Prompt | [`nmaiex_mapper_service.py`](../app/services/nmaiex_mapper_service.py#L211-L225) | 211–225 | tiếng Việt | Hằng số cấp module `_PROFICIENCY_SYSTEM_PROMPT` |
| P9 | Synthetic CV Generation Prompt | [`synthetic_data/prompts.py`](../synthetic_data/prompts.py#L27-L47) | 27–47 | tiếng Việt | Chỉ dùng làm công cụ dev |
| P10 | Synthetic Job Generation Prompt | [`synthetic_data/prompts.py`](../synthetic_data/prompts.py#L94-L123) | 94–123 | tiếng Việt | Chỉ dùng làm công cụ dev |
...
|---|---|---|---|
| [`ParsedCV`](../app/models/cv_models.py#L97) | `cv_models.py` | UC-1 (CV Parse) | Gemini native schema, OpenAI json_schema (strict:False), Anthropic schema-in-prompt + `model_validate_json` |
| [`ParserSelfReport`](../app/models/cv_models.py#L78) | `cv_models.py` | UC-1 (quality gate) | Lồng trong ParsedCV (Nested in ParsedCV) |
| [`LanguageEntry`](../app/models/cv_models.py#L60) | `cv_models.py` | UC-1 + UC-10 | Lồng trong ParsedCV (Nested in ParsedCV) |
| [`CandidateInfo`](../app/models/cv_models.py#L18) | `cv_models.py` | UC-1 (identity check) | Lồng trong ParsedCV (Nested in ParsedCV) |
| [`ChatQueryRequest`](../app/models/chat.py#L14) | `chat.py` | UC-4 (input) | Xác thực request bằng Pydantic (Pydantic request validation) |
| [`ChatQueryResponse`](../app/models/chat.py#L50) | `chat.py` | UC-4 (output) | Tuần tự hóa phản hồi bằng Pydantic (Pydantic response serialization) |
| [`ContextWarning`](../app/models/chat.py#L39) | `chat.py` | UC-4 (budget) | Lồng trong ChatQueryResponse |
| [`SkillMappingResult`](../app/models/nmaiex_schemas.py#L15) | `nmaiex_schemas.py` | UC-8 (skill mapping) | `model_validate_json` kết hợp loại bỏ các markdown fence |
| [`ProvinceMappingResult`](../app/models/nmaiex_schemas.py#L26) | `nmaiex_schemas.py` | UC-7 (province mapping) | Pydantic constructor |
| [`ScoreBreakdown`](../app/models/nmaiex_schemas.py#L40) | `nmaiex_schemas.py` | UC-11/12 (ranking) | Kết quả chấm điểm có tính deterministic |
| [`CandidateRankResult`](../app/models/nmaiex_schemas.py#L63) | `nmaiex_schemas.py` | UC-12 (J→C ranking) | Deterministic |
| [`GenerationTrace`](../app/services/rag_orchestrator.py#L66) | `rag_orchestrator.py` | UC-4/5/6 (generation) | Dataclass (không phải Pydantic) |
