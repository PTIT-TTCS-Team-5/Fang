...
def _build_full_cv_system_prompt(
    ...
    if app_ctx.job_posting:
        ...
        location_parts = [
            str(jp[k]) for k in ("workloc", "provincename") if jp.get(k)
        ]
        if location_parts:
    ...
    if app_ctx.emails:
        email_lines = [
            "[UNTRUSTED EMAIL — LỊCH SỬ EMAIL (gần nhất, body đã cắt)]"
        ]
...
async def process_chat_query(
    ...
    # --- 9. Persist assistant message + audit log ---
