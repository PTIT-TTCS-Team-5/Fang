...
| 6 | Chat full-CV chỉ hard-gate `CVPARSED` usable, không hard-gate `AIINDEXJOB.SUCCESS` | Luồng chat mới không còn phụ thuộc vector chunks; ingestion status chỉ còn là telemetry/ops |
...
History đưa vào budget/messages đã được lọc: giữ system summary messages, giữ
user/assistant turns chưa summarized, và loại bỏ các user/assistant turns đã
`summarized=True` để thao tác "Tóm tắt & tiếp tục" thật sự giảm context.
...
- Single-application chat UI phải dùng `CVPARSED` readiness thay vì hard-gate `AIINDEXJOB.SUCCESS`.
- Khi `topK = 0`, UI hiển thị `"Full CV context"` thay vì `"top-0 chunks"` và không dùng wording `"RAG pipeline"`.
...
- `_filter_history_for_full_context(history, current_user_message_id)` — giữ system summaries, bỏ turns đã summarized.
...
- Module boundary: no `embed_chunks` import, `_vector_search` kept, no `AIINDEXJOB` hard gate, `process_chat_query` source contract.
