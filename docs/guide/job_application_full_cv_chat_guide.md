...
Frontend `miCareer-mini/app.py` (ở thời điểm Phase 1) hardcode wording `"📚 top-{topK} chunks"`. Sau khi chuyển full-CV, `topK` luôn = 0 → kỳ. Patch frontend:
...
(Patch đề xuất: kiểm tra `result.get("topK", 0) > 0` mới hiển thị, ngược lại "Full CV context".)
